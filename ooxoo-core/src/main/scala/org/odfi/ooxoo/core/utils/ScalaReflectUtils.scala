package org.odfi.ooxoo.core.utils
...
trait ReflectUtilsTrait {
  ...
  def listElementAndAttributeFields = {
    this.getFields(this).collect {

      case f if (f.getAnnotationsByType(classOf[xelement]).length > 0) =>

        var elementbase = xelement_base(f)
        (elementbase.name, f)

      case f if (f.getAnnotationsByType(classOf[xattribute]).length > 0) =>
