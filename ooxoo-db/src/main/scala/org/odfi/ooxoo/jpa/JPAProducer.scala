...
class JPAProducer extends ModelProducer {
    ...
    def registerModels = {
      org.odfi.ooxoo.hibernate.OOXOOHibernate.registerPackage(getClass.getPackage)
...
"""
        ...
        def writeElement(element: Element): Unit = {
            ...
            //--------------
            if (!element.isTrait) {
                packageFileWriter << s""" org.odfi.ooxoo.hibernate.OOXOOHibernate.registerModel("$targetPackage.$className")  """
            }
            // Write File
            //-----------------------
